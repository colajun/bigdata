...
comment_table_name = "haoqixin_comment_news_"+ time.strftime("%Y_%m")
...
haoqixin_comment_table_insert = "replace into " + comment_table_name + "(news_id, comment_id, agree_count, comment_time, comment_timestamp, content, parent_content) VALUES(%s, %s, %s, %s, %s, %s, %s)"
...
class IfengCommentCollector(object):
    ...
    def GetNewsCommentCount(self, documentId):
        url = url = "http://app3.qdaily.com/app3/comments/index/article/" + str(documentId) + "/0.json"
        print  url
        try:
            ...
            counter=0
            for comment in content["response"]["comments"]:
                counter+=1
            # print counter
            return counter
    ...
    # def GetNewsCommentCount(self, documentId):
    #
		# url = url = "http://app3.qdaily.com/app3/comments/article/"+str(documentId)+"/0.json"
    #
		# try:
    #
		# 	response = urllib2.urlopen(url)
    ...
    def GetNewsCommentByCommentsUrl(self,commentsUrl, documentId, increment):
        url = url = "http://app3.qdaily.com/app3/comments/index/article/" + str(documentId) + "/"+str(increment)+".json"
        ...
        try:
            ...
            for comment in content["response"]["comments"]:
                comment_id = comment["author"]["id"]
                comment_uptimes = comment["praise_count"]
                publishTimestamp = comment["publish_time"]
                publishTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(publishTimestamp))
                comment_contents = comment["content"]
                parent_content=""
                if comment["child_comments"] != None:
                    for child in comment["child_comments"]:
                        parent_content = ",,,,,,,"+child["content"]

                try:
                    self.cursor.execute(haoqixin_comment_table_insert, (
                                documentId, comment_id, comment_uptimes,
                                publishTime, publishTimestamp, comment_contents,
                                parent_content))
                    self.con.commit()
                except Exception,e:
                    print e
                # print  comment_contents
            if content["response"]["has_more"] ==True:
                increment = content["response"]["last_key"]
                # print increment
                self.GetNewsCommentByCommentsUrl(commentsUrl, documentId, increment)
